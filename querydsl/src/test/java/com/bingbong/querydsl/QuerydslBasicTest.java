...
class QuerydslBasicTest {
    ...
    void searchAndComma() {
        Member findMember = queryFactory
            .selectFrom(member)
            .where(
                member.username.eq("memeber1"), // Comma가 and로 치환된다.
    ...
    void resultFetch() {
        ...
        QueryResults<Member> results = queryFactory
            .selectFrom(member)
            .fetchResults(); // 카운트 쿼리와 같이 나가기 때문에 2개의 SQL문이 나간다.
    ...
    // 연관관계가 없어도 조인가능함. 그게 세타조인
    // 회원 이름과 팀 이름이 같은 회언 조회
    @Test
    void thetaJoin() {
    ...
    void joinOnFiltering() {
        // Tuple로 나온 이유는 select에 member와 team을 조회했기 때문
        List<Tuple> result = queryFactory
            .select(member, team)
            .from(member)
    ...
    void joinOnNoRelation() {
        ...
        List<Tuple> result = queryFactory
            .select(member, team)
            .from(member)
//            .leftJoin(member.team, team).on(member.username.eq(team.name)) // 이 방식으로 조인하면 FK값에 맞는 조인을 한 후, and로 필터링한다.
            .leftJoin(team).on(member.username.eq(team.name)) // 빼버리면 on 절에 있는 이름으로만 조인이 된다.
            .fetch();
